...
def send_login_email(
    ...
    try:
        ...
        logger.info(
            f"Рендеринг шаблона email с переменными: user_name={template_vars['user_name']}, login_time={template_vars['login_time']}"
        )

        # Загружаем и рендерим шаблоны
        try:
            text_content = template_loader.render_template(
                "login_email.txt", **template_vars
            )
            html_content = template_loader.render_template(
                "login_email.html", **template_vars
            )

            # Проверяем, что переменные подставились
            if "{{user_name}}" in html_content or "{{login_time}}" in html_content:
                logger.warning(
                    "Переменные не были подставлены в шаблон! Проверьте шаблон."
                )
                logger.debug(
                    f"HTML контент (первые 500 символов): {html_content[:500]}"
                )
    ...
    try:
        ...
        if payment_time:
            try:
                ...
                logger.warning(
                    f"Не удалось распарсить время пополнения '{payment_time}': {e}"
                )
                payment_time_str = payment_time
        else:
            from datetime import datetime
        ...
        # Логируем переменные для отладки
        logger.info(
            f"Рендеринг шаблона email о пополнении баланса: user_name={template_vars['user_name']}, amount={template_vars['amount']}"
        )

        # Загружаем и рендерим шаблоны
        try:
            text_content = template_loader.render_template(
                "balance_email.txt", **template_vars
            )
            html_content = template_loader.render_template(
                "balance_email.html", **template_vars
            )
