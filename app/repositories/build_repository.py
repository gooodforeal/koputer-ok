...
from app.exceptions.builds import BuildValidationError, ComponentCategoryError
...
def validate_component_categories(components: List[Component]) -> None:
    ...
    if missing_categories:
        missing_names = [
            CATEGORY_NAMES.get(cat, str(cat)) for cat in missing_categories
        ]
        raise ComponentCategoryError(
...
class BuildRepository:
    ...
    async def create(self, build_data: BuildCreate, author_id: int) -> Build:
        ...
        if not build_data.component_ids or len(build_data.component_ids) == 0:
            raise BuildValidationError(detail="Сборка должна содержать компоненты")
        ...
        await self.db.execute(
            insert(build_components).values(
                [
                    {"build_id": db_build.id, "component_id": comp.id}
    ...
    async def update(self, build: Build, build_data: BuildUpdate) -> Build:
        ...
        if component_ids is not None:
            ...
            await self.db.execute(
                insert(build_components).values(
                    [
                        {"build_id": build.id, "component_id": comp.id}
                        for comp in components
                    ]
                )
            )
        ...
        if user_id is not None:
            ...
            existing_view_result = await self.db.execute(
                select(BuildView).filter(
                    BuildView.build_id == build_id, BuildView.user_id == user_id
                )
            )
    ...
    async def search(
        ...
        if query:
            search_condition = or_(
                Build.title.ilike(f"%{query}%"), Build.description.ilike(f"%{query}%")
            )
    ...
    async def count_search_results(
        self, query: str = "", author_id: Optional[int] = None
    ) -> int:
    ...
    async def get_user_rating(
        self, build_id: int, user_id: int
    ) -> Optional[BuildRating]:
        """Получить оценку пользователя для конкретной сборки"""
        result = await self.db.execute(
            select(BuildRating).filter(
                BuildRating.build_id == build_id, BuildRating.user_id == user_id
            )
        )
        return result.scalar_one_or_none()

    async def create_rating(
        self, build_id: int, user_id: int, rating_data: BuildRatingCreate
    ) -> BuildRating:
        """Создать оценку"""
        db_rating = BuildRating(
            build_id=build_id, user_id=user_id, score=rating_data.score
        )
    ...
    async def get_build_comments(
        ...
        result = await self.db.execute(
            ...
            .filter(BuildComment.build_id == build_id, BuildComment.parent_id == None)
    ...
    async def count_build_comments(self, build_id: int) -> int:
        ...
        result = await self.db.execute(
            select(func.count(BuildComment.id)).filter(
    ...
    async def create_comment(
        self, build_id: int, user_id: int, comment_data: BuildCommentCreate
    ) -> BuildComment:
        """Создать комментарий"""
