...
class ComponentRepository:
    ...
    async def get_by_category(
        self, category: ComponentCategory, skip: int = 0, limit: int = 100
    ) -> List[Component]:
    ...
    async def create(
        ...
        component = Component(
            name=name, link=link, price=price, image=image, category=category
        )
    ...
    async def delete_by_category(self, category: ComponentCategory) -> int:
        ...
        try:
            result = await self.db.execute(
                select(Component).filter(Component.category == category)
            )
            components = list(result.scalars().all())
            count = len(components)
            for component in components:
    ...
    async def count_by_category(self, category: ComponentCategory) -> int:
        ...
        result = await self.db.execute(
            select(func.count(Component.id)).filter(Component.category == category)
        )
        return result.scalar() or 0
