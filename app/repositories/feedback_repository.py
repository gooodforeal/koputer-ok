...
class FeedbackRepository(BaseRepository[Feedback]):
    ...
    async def create_feedback(
        self, feedback_data: FeedbackCreate, user_id: int
    ) -> Feedback:
        """Создать новый отзыв"""
        feedback = Feedback(
    ...
    async def update_feedback(
        self, feedback_id: int, feedback_data: FeedbackUpdate
    ) -> Optional[Feedback]:
    ...
    async def admin_update_feedback(
        ...
        if (
            update_data.get("status") == FeedbackStatus.IN_PROGRESS
            and "assigned_to_id" not in update_data
