...
class UserRepository:
    ...
    async def create(self, user_data: UserCreate) -> User:
        ...
        # Очищаем кэш для этого пользователя (ленивый импорт для избежания циклических зависимостей)
        try:
            from app.dependencies.auth import clear_user_cache
            await clear_user_cache(
                user_email=db_user.email, telegram_id=db_user.telegram_id
            )
        except (ImportError, Exception):
            # Если импорт не удался или произошла ошибка, игнорируем
            pass
        ...
        try:
            ...
            if existing_user:
                return await self.update(
                    existing_user, name=user_data.name, picture=user_data.picture
                )
    ...
    async def get_admins(self, skip: int = 0, limit: int = 100) -> List[User]:
        ...
        result = await self.db.execute(
            select(User)
            .filter((User.role == UserRole.ADMIN) | (User.role == UserRole.SUPER_ADMIN))
            .offset(skip)
            .limit(limit)
        )
        return list(result.scalars().all())
