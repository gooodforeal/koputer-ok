...
async def start_parsing(
    ...
    clear_existing: bool = Query(
        True, description="Очистить существующие данные перед парсингом"
    ),
    current_user: User = Depends(require_admin_or_super_admin),
    component_repo: ComponentRepository = Depends(get_component_repository),
    component_parser_service: ComponentParserService = Depends(
        get_component_parser_service
    ),
):
    """
    ...
    if parse_status.get("is_running", False):
        raise ComponentParseAlreadyRunningError()
    ...
    background_tasks.add_task(
        component_parser_service.start_parsing, component_repo, clear_existing
    )

    return ParseStartResponse(
        message="Парсинг запущен в фоновом режиме", status="started"
    )
...
async def stop_parsing(
    ...
    if not stopped:
        raise ComponentParseNotRunningError()
...
async def get_components_by_category(
    ...
    try:
        ...
        raise InvalidComponentCategoryError(category=category)

    # Получаем компоненты по категории с поиском
    components = await component_repo.get_by_category_with_search(
        category=component_category, query=query, skip=skip, limit=limit
    )
