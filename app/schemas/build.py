...
class BuildBase(BaseModel):
    ...
    description: str = Field(
        ..., min_length=10, max_length=5000, description="Описание сборки"
    )
    component_ids: List[int] = Field(
        default=[], description="Список ID компонентов из таблицы Components"
    )
...
class BuildUpdate(BaseModel):
    ...
    component_ids: Optional[List[int]] = Field(
        None, description="Список ID компонентов из таблицы Components"
    )
...
class BuildResponse(BuildBase):
    ...
    components: List[ComponentResponse] = Field(
        default=[], description="Список компонентов сборки"
    )
    views_count: int
    average_rating: float
    ratings_count: int
    total_price: float = Field(
        default=0.0, description="Общая стоимость сборки из суммы цен компонентов"
    )
    created_at: datetime
...
class BuildCommentCreate(BaseModel):
    ...
    content: str = Field(
        ..., min_length=1, max_length=2000, description="Текст комментария"
    )
    parent_id: Optional[int] = Field(
        None, description="ID родительского комментария для ответа"
    )
