...
class BuildService:
    ...
    async def create_build(
        self, build_data: BuildCreate, author_id: int
    ) -> BuildResponse:
        """
    ...
    async def get_top_builds(self, limit: int = 10) -> BuildTopResponse:
        ...
        return BuildTopResponse(builds=builds, total=len(builds))
    ...
    async def update_build(
        self, build_id: int, build_data: BuildUpdate, current_user: User
    ) -> BuildResponse:
        """
        Обновить сборку
    ...
    async def delete_build(self, build_id: int, current_user: User) -> MessageResponse:
        ...
        if not success:
            raise BuildNotFoundError(detail="Ошибка при удалении сборки")
    ...
    async def create_rating(
        self, build_id: int, rating_data: BuildRatingCreate, current_user: User
    ) -> BuildRatingResponse:
        """
        ...
        existing_rating = await self.build_repo.get_user_rating(
            build_id, current_user.id
        )
        if existing_rating:
            raise RatingAlreadyExistsError()

        return await self.build_repo.create_rating(
            build_id, current_user.id, rating_data
        )

    async def update_rating(
        self, build_id: int, rating_data: BuildRatingUpdate, current_user: User
    ) -> BuildRatingResponse:
        """
    ...
    async def delete_rating(self, build_id: int, current_user: User) -> MessageResponse:
        """
        ...
        if not success:
            raise RatingNotFoundError(detail="Ошибка при удалении оценки")

        return MessageResponse(message="Оценка успешно удалена")

    async def get_user_rating(
        self, build_id: int, current_user: User
    ) -> BuildRatingResponse:
        """
        ...
        rating = await self.build_repo.get_user_rating(build_id, current_user.id)
        if not rating:
            raise RatingNotFoundError(detail="Вы еще не оценили эту сборку")
    ...
    async def create_comment(
        self, build_id: int, comment_data: BuildCommentCreate, current_user: User
    ) -> BuildCommentSingleResponse:
        """
        ...
        if comment_data.parent_id:
            parent_comment = await self.build_repo.get_comment_by_id(
                comment_data.parent_id
            )
            if not parent_comment or parent_comment.build_id != build_id:
                raise CommentNotFoundError(detail="Родительский комментарий не найден")
        ...
        return await self.build_repo.create_comment(
            build_id, current_user.id, comment_data
        )

    async def get_comments(
        self, build_id: int, skip: int = 0, limit: int = 50
    ) -> BuildCommentListResponse:
        """
        ...
        return BuildCommentListResponse(comments=comments, total=total)
    ...
    async def delete_comment(
        self, build_id: int, comment_id: int, current_user: User
    ) -> MessageResponse:
        """
        Удалить комментарий
        ...
        if not success:
            raise CommentNotFoundError(detail="Ошибка при удалении комментария")
    ...
    async def export_build_pdf(self, build_id: int) -> StreamingResponse:
        ...
        try:
            ...
            safe_title = safe_filename(
                build.title, max_length=80, prefix=f"build_{build_id}_"
            )
            ...
            raise BuildValidationError(detail=f"Ошибка при генерации PDF: {str(e)}")
