...
class ShopParser:
    ...
    def _get_cache_filename(self, url: str) -> str:
        ...
        safe_url = re.sub(
            r"[^\w\-_\.]", "_", url.replace("https://", "").replace("http://", "")
        )
    ...
    def _load_from_cache(self, cache_file: str) -> Optional[str]:
        ...
        if os.path.exists(cache_file):
            try:
                ...
                logger.warning(
                    f"[Предупреждение] Ошибка при чтении кеша {cache_file}: {e}"
                )
                return None
        return None
    ...
    def _save_to_cache(self, cache_file: str, html: str):
        ...
        try:
            ...
            logger.warning(
                f"[Предупреждение] Ошибка при сохранении кеша {cache_file}: {e}"
            )
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(
                        f"[Инфо] Страница загружена: {url} (размер: {len(html)} символов)"
                    )
                    ...
                    logger.error(
                        f"[Диагностика] Заголовки ответа: {dict(response.headers)}"
                    )
                    logger.error(
                        f"[Диагностика] Тело ответа (первые 1000 символов): {error_body[:1000]}"
                    )
                    return None
                else:
                    error_body = await response.text()
                    logger.warning(
                        f"[Предупреждение] HTTP статус {response.status} для {url}"
                    )
                    logger.warning(
                        f"[Диагностика] Тело ответа (первые 500 символов): {error_body[:500]}"
                    )
                    return None
    ...
    async def _get_products_from_page(self, html: str) -> List[Dict[str, str]]:
        ...
        for item in items:
            try:
                ...
                link = self.base_url + item.find(
                    "div", class_="products__content"
                ).find("a").get("href")
                price = (
                    item.find("div", class_="products__content")
                    .find("div", class_="products__prices")
                    .find("div", class_="prices__price")
                    .text
                )
                image = self.base_url + item.find(
                    "div", class_="products__img-wrap"
                ).find("a").find("span").find("img").get("src")
            except Exception as e:
    ...
    async def get_products_from_category(
        self, category_name: str | ComponentsCategory
    ) -> List[Dict[str, str]]:
        """
        ...
        if isinstance(category_name, ComponentsCategory):
            ...
            for cat in ComponentsCategory:
                if (
                    ...
                    or cat.name.lower() == category_name.lower()
                ):
                    category = cat
                    break
            ...
            if category is None:
                ...
                logger.warning(
                    f"[Предупреждение] Категория '{category_name}' не найдена, используем Процессоры"
                )
...
async def get_products_by_category(
    category_name: str | ComponentsCategory,
) -> List[Dict[str, str]]:
    """
...
async def main():
    ...
    try:
        ...
        logger.error("[Ошибка] Не удалось загрузить страницу")
    finally:
        await parser.close()
