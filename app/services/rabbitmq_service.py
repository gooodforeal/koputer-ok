...
class RabbitMQService:
    ...
    async def declare_exchange(
        ...
        exchange = await self.channel.declare_exchange(
            exchange_name, exchange_type.value, durable=durable
        )
    ...
    async def bind_queue(
        self, queue: aio_pika.Queue, exchange: aio_pika.Exchange, routing_key: str = ""
    ) -> None:
        """
        ...
        logger.debug(
            f"Очередь '{queue.name}' привязана к exchange '{exchange.name}' с routing_key '{routing_key}'"
        )
    ...
    async def publish_message(
        ...
        logger.debug(
            f"Сообщение опубликовано в exchange '{exchange.name}' с routing_key '{routing_key}'"
        )
    ...
    async def setup_celery_queue(
        self, queue_name: str, exchange_name: str = "celery"
