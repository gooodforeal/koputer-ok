...
class FeedbackService:
    ...
    async def create_feedback(
        self, feedback_data: FeedbackCreate, current_user: User
    ) -> FeedbackResponse:
        """
        Создать новый отзыв
    ...
    async def get_public_feedbacks(
        ...
        feedbacks = await self.feedback_repo.get_all_feedbacks(
            skip=skip, limit=limit, status=None, feedback_type=None
        )
        total = await self.feedback_repo.count_all_feedbacks(
            status=None, feedback_type=None
        )
    ...
    async def get_feedbacks(
        ...
        feedbacks = await self.feedback_repo.get_all_feedbacks(
            skip=skip, limit=limit, status=status_filter, feedback_type=type_filter
        )
        total = await self.feedback_repo.count_all_feedbacks(
            status=status_filter, feedback_type=type_filter
        )
    ...
    async def get_user_feedbacks(
        ...
        Returns:
            FeedbackListResponse со списком отзывов
        """
        feedbacks = await self.feedback_repo.get_user_feedbacks(
    ...
    async def get_assigned_feedbacks(
        ...
        Returns:
            FeedbackListResponse со списком отзывов
        """
        feedbacks = await self.feedback_repo.get_assigned_feedbacks(
            current_user.id, skip, limit
        )
    ...
    async def update_feedback(
        self, feedback_id: int, feedback_data: FeedbackUpdate, current_user: User
    ) -> FeedbackResponse:
        """
        Обновить свой отзыв
    ...
    async def admin_update_feedback(
        ...
        updated_feedback = await self.feedback_repo.admin_update_feedback(
            feedback_id, feedback_data, current_user.id
        )
        return updated_feedback

    async def delete_feedback(self, feedback_id: int, current_user: User) -> None:
        """
        Удалить отзыв
        ...
        if feedback.user_id != current_user.id and current_user.role not in [
            ...
            raise FeedbackAccessDeniedError(detail="Нет прав на удаление этого отзыва")

        success = await self.feedback_repo.delete_feedback(feedback_id)
        if not success:
            raise FeedbackNotFoundError(detail="Ошибка при удалении отзыва")
