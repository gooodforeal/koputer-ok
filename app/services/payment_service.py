...
from app.exceptions.users import InsufficientPermissionsError
...
class PaymentService:
    ...
    async def create_payment(
        self, user_id: int, payment_data: PaymentCreate
    ) -> PaymentResponse:
        """
        ...
        try:
            payment_info = await self.yookassa_service.create_payment(
                ...
                metadata={"transaction_id": transaction.id, "balance_id": balance.id},
            )
            ...
            raise PaymentCreationError(detail=f"Ошибка при создании платежа: {str(e)}")
    ...
    async def process_webhook(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Found transaction {transaction.id} with status {transaction.status}"
            )
    ...
    async def sync_payment_status(
        ...
        if transaction.user_id != user_id:
            raise InsufficientPermissionsError(detail="Доступ запрещен")
