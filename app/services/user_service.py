...
from app.exceptions.users import UserNotFoundError, InsufficientPermissionsError
...
class UserService:
    ...
    async def search_users(
        ...
        users = await self.user_repo.search_users(
            query=q, skip=skip, limit=per_page, role=role, is_active=is_active
        )

        # Получаем общее количество для пагинации
        total = await self.user_repo.count_search_results(
            query=q, role=role, is_active=is_active
        )
    ...
    async def update_user_profile(
        self, profile_update: UserUpdate, current_user: User
    ) -> UserResponse:
        """
    ...
    async def update_user_role(
        self, user_id: int, role_update: UserUpdate
    ) -> UserResponse:
        """
