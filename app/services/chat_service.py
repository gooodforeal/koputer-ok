...
class ChatService:
    ...
    async def create_chat(
        self, chat_data: ChatCreate, current_user: User
    ) -> ChatResponse:
        """
        Создать новый чат
    ...
    async def get_chat(self, chat_id: int, current_user: User) -> ChatResponse:
        """
        Получить чат по ID
        ...
        if (
            ...
            and chat.admin_id != current_user.id
            and current_user.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]
        ):
            raise ChatAccessDeniedError(detail="Нет доступа к этому чату")

        return chat

    async def assign_admin_to_chat(
        self, chat_id: int, request: AssignAdminRequest, current_user: User
    ) -> ChatResponse:
        """
    ...
    async def send_message(
        self, chat_id: int, message_data: MessageCreate, current_user: User
    ) -> MessageResponse:
        """
        ...
        return await self.chat_repo.create_message(
            message_data_with_chat, current_user.id
        )

    async def get_chat_messages(
        self, chat_id: int, limit: int, offset: int, current_user: User
    ...
    async def mark_chat_as_read(self, chat_id: int, current_user: User) -> dict:
        """
    ...
    async def get_admin_chats_summary(self, current_user: User) -> List[dict]:
        ...
        if current_user.role == UserRole.SUPER_ADMIN:
            ...
            summaries = []
            for chat in chats:
                unread_count = await self.chat_repo.get_unread_count(
    ...
    async def update_chat_status(
        self, chat_id: int, request: UpdateChatStatusRequest, current_user: User
    ) -> ChatResponse:
        """
        ...
        # Проверяем права доступа
        if current_user.role == UserRole.ADMIN and chat.admin_id != current_user.id:
            raise ChatAccessDeniedError(
                detail="Вы можете изменять статус только своих чатов"
            )

        updated_chat = await self.chat_repo.update_chat_status(
            chat_id, request.status, current_user.id
        )
        return updated_chat

    async def close_chat(self, chat_id: int, current_user: User) -> ChatResponse:
        """
        Закрыть чат
        ...
        # Проверяем права доступа
        if current_user.role == UserRole.ADMIN and chat.admin_id != current_user.id:
            raise ChatAccessDeniedError(detail="Вы можете закрывать только свои чаты")

        closed_chat = await self.chat_repo.close_chat(chat_id, current_user.id)
        return closed_chat

    async def reopen_chat(self, chat_id: int, current_user: User) -> ChatResponse:
        """
        Переоткрыть чат
    ...
    async def start_working_on_chat(
        ...
        working_chat = await self.chat_repo.start_working_on_chat(
            chat_id, current_user.id
        )
        return working_chat

    async def get_chats_by_status(
        self, status: ChatStatus, current_user: User
        ...
        if current_user.role == UserRole.SUPER_ADMIN:
            ...
            chats = await self.chat_repo.get_admin_chats_by_status(
