...
class YooKassaService:
    ...
    async def create_payment(
        ...
        payment_metadata = {"user_id": user_id, **(metadata or {})}
    ...
    def parse_webhook(webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                if hasattr(payment_object, "status")
                else None,
                "paid": paid,
                "cancelled": cancelled,
                "amount": Decimal(payment_object.amount.value)
                if hasattr(payment_object, "amount") and payment_object.amount
                else Decimal("0"),
                "metadata": payment_object.metadata
                if hasattr(payment_object, "metadata")
                else {},
                "created_at": created_at,
            }
    ...
    def is_payment_cancelled(webhook_data: Dict[str, Any]) -> bool:
        ...
        """
        parsed = YooKassaService.parse_webhook(webhook_data)
        return parsed.get("event") == "payment.cancelled" or parsed.get(
            "cancelled", False
        )
