...
class AuthService:
    ...
    async def _get_google_user_info(self, access_token: str) -> GoogleUserInfo:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise GoogleAuthError(detail=f"Error getting user info: {str(e)}")
            ...
            if user.email:
                try:
                    ...
                    await self.email_publisher.publish_login_email(
                        email=user.email, user_name=user.name, login_time=login_time
                    )
                except Exception as e:
    ...
    async def authorize_telegram_user(
        self, request: TelegramAuthorizeRequest
    ) -> TelegramAuthorizeResponse:
        """
        ...
        """
        logger.info(
            f"Обработка авторизации через Telegram для пользователя {request.telegram_id} с токеном {request.auth_token[:10]}..."
        )
        ...
        if not success:
            logger.warning(
                f"Не удалось связать токен {request.auth_token[:10]}... с пользователем {request.telegram_id}"
            )
            raise FailedToLinkTokenError()
        ...
        token_updated = await self.auth_token_storage.update_token_data(
            request.auth_token, jwt_token=jwt_token
        )
        logger.info(
            f"JWT токен сохранен для токена {request.auth_token[:10]}...: {token_updated}"
        )

        if not token_updated:
            logger.error(
                f"Не удалось сохранить JWT токен для токена {request.auth_token[:10]}..."
            )
            raise FailedToSaveTokenError()
        ...
        if db_user.email:
            try:
                login_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                await self.email_publisher.publish_login_email(
                    email=db_user.email, user_name=db_user.name, login_time=login_time
                )
            except Exception as e:
                # Логируем ошибку, но не прерываем процесс входа
                logger.error(f"Ошибка при отправке email при входе: {str(e)}")
        ...
        return TelegramAuthorizeResponse(
            status="success", message="Authorization completed"
        )
    ...
    async def check_telegram_auth(self, auth_token: str) -> TelegramAuthCheckResponse:
        ...
        logger.info(
            f"Проверка токена {auth_token[:10]}...: used={used}, has_jwt={has_jwt}, telegram_id={telegram_id}"
        )
        ...
        if used and telegram_id:
            ...
            if jwt_token:
                ...
                logger.warning(
                    f"Токен {auth_token[:10]}... используется, но JWT токен отсутствует!"
                )
