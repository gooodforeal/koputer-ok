...
class TestCreateChat:
    ...
    async def test_create_chat_success(self, client, test_user):
        ...
        chat_data = {"user_id": test_user.id, "status": "OPEN"}
    ...
    async def test_create_chat_when_exists(self, client, test_user, db_session):
        ...
        existing_chat = await chat_repo.create_chat(
            ChatCreate(user_id=test_user.id, status=ChatStatus.OPEN)
        )
    ...
    @pytest.mark.asyncio
    async def test_create_chat_for_other_user_forbidden(
        self, client, test_user, test_user2
    ):
        """Тест создания чата для другого пользователя"""
        chat_data = {"user_id": test_user2.id, "status": "OPEN"}
...
class TestGetMyChat:
    ...
    async def test_get_my_chat_not_found(self, client):
        ...
        assert "chat not found" in response.json()["detail"].lower()
...
class TestGetChat:
    ...
    @pytest.mark.asyncio
    async def test_get_chat_forbidden_other_user(
        self, client, test_user, test_user2, db_session
    ):
        """Тест получения чужого чата"""
        chat_repo = ChatRepository(db_session)
        chat = await chat_repo.create_chat(
            ChatCreate(user_id=test_user2.id, status=ChatStatus.OPEN)
        )
        await db_session.flush()
    ...
    @pytest.mark.asyncio
    async def test_get_chat_by_admin_success(
        self, admin_client, test_user, test_admin, db_session
    ):
...
class TestGetAdminChats:
    ...
    @pytest.mark.asyncio
    async def test_get_admin_chats_super_admin(
        self, super_admin_client, test_user, db_session
    ):
...
class TestAssignAdminToChat:
    ...
    async def test_assign_admin_success(
        ...
        assign_data = {"admin_id": test_admin.id}
    ...
    @pytest.mark.asyncio
    async def test_assign_admin_forbidden_for_user(
        self, client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_assign_admin_unauthenticated(
        self, unauthenticated_client, test_user, db_session
    ):
        ...
        assign_data = {"admin_id": 1}

        response = unauthenticated_client.post(
            f"/api/chat/{chat.id}/assign", json=assign_data
        )
...
class TestSendMessage:
    ...
    async def test_send_message_chat_not_found(self, client):
        ...
        message_data = {"content": "Тестовое сообщение", "message_type": "text"}
    ...
    @pytest.mark.asyncio
    async def test_send_message_by_admin(
        self, admin_client, test_user, test_admin, db_session
    ):
        ...
        message_data = {"content": "Ответ администратора", "message_type": "text"}
    ...
    async def test_send_message_unauthenticated(
        ...
        response = unauthenticated_client.post(
            f"/api/chat/{chat.id}/messages", json=message_data
        )
...
class TestGetChatMessages:
    ...
    async def test_get_messages_success(self, client, test_user, db_session):
        ...
        await chat_repo.create_message(
            MessageCreateWithChatId(
                chat_id=chat.id, content="Первое сообщение", message_type="text"
            ),
            test_user.id,
        )
        await chat_repo.create_message(
            MessageCreateWithChatId(
                chat_id=chat.id, content="Второе сообщение", message_type="text"
            ),
            test_user.id,
        )
        await db_session.flush()
    ...
    async def test_get_messages_with_pagination(self, client, test_user, db_session):
        ...
        for i in range(5):
            await chat_repo.create_message(
                MessageCreateWithChatId(
                    chat_id=chat.id, content=f"Сообщение {i + 1}", message_type="text"
                ),
                test_user.id,
            )
        await db_session.flush()
...
class TestMarkChatAsRead:
    ...
    @pytest.mark.asyncio
    async def test_mark_as_read_unauthenticated(
        self, unauthenticated_client, test_user, db_session
    ):
...
class TestGetAdminChatsSummary:
    ...
    @pytest.mark.asyncio
    async def test_get_admin_chats_summary_success(
        self, admin_client, test_user, test_admin, db_session
    ):
        """Тест успешного получения краткой информации"""
        chat_repo = ChatRepository(db_session)
    ...
    @pytest.mark.asyncio
    async def test_get_admin_chats_summary_super_admin(
        self, super_admin_client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_admin_chats_summary_unauthenticated(
...
class TestUpdateChatStatus:
    ...
    @pytest.mark.asyncio
    async def test_update_status_forbidden_admin_other_chat(
        self, admin_client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_status_super_admin_success(
        self, super_admin_client, test_user, db_session
    ):
        ...
        response = super_admin_client.patch(
            f"/api/chat/{chat.id}/status", json=update_data
        )
    ...
    @pytest.mark.asyncio
    async def test_update_status_forbidden_for_user(
    ...
    @pytest.mark.asyncio
    async def test_update_status_unauthenticated(
        self, unauthenticated_client, test_user, db_session
    ):
...
class TestCloseChat:
    ...
    @pytest.mark.asyncio
    async def test_close_chat_success(
        self, admin_client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_close_chat_forbidden_admin_other_chat(
        self, admin_client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_close_chat_super_admin_success(
        self, super_admin_client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_close_chat_unauthenticated(
        self, unauthenticated_client, test_user, db_session
    ):
...
class TestReopenChat:
    ...
    @pytest.mark.asyncio
    async def test_reopen_chat_success(
        self, admin_client, test_user, test_admin, db_session
    ):
        """Тест успешного переоткрытия чата"""
        chat_repo = ChatRepository(db_session)
        chat = await chat_repo.create_chat(
            ChatCreate(user_id=test_user.id, status=ChatStatus.CLOSED)
        )
        await chat_repo.assign_admin_to_chat(chat.id, test_admin.id)
        await db_session.flush()
    ...
    @pytest.mark.asyncio
    async def test_reopen_chat_forbidden_admin_other_chat(
        self, admin_client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_reopen_chat_super_admin_success(
        self, super_admin_client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_reopen_chat_unauthenticated(
        self, unauthenticated_client, test_user, db_session
    ):
...
class TestStartWorkingOnChat:
    ...
    @pytest.mark.asyncio
    async def test_start_working_success(
        self, admin_client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_start_working_with_assigned_admin(
        self, admin_client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_start_working_forbidden_admin_other_chat(
        self, admin_client, test_user, test_admin, test_user2, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_start_working_super_admin_success(
        self, super_admin_client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_start_working_forbidden_for_user(
        self, client, test_user, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_start_working_unauthenticated(
        self, unauthenticated_client, test_user, db_session
    ):
...
class TestGetChatsByStatus:
    ...
    @pytest.mark.asyncio
    async def test_get_chats_by_status_open(
        self, admin_client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_chats_by_status_in_progress(
        self, admin_client, test_user, test_admin, db_session
    ):
        ...
        await chat_repo.assign_admin_to_chat(chat.id, test_admin.id)
        await chat_repo.update_chat_status(
            chat.id, ChatStatus.IN_PROGRESS, test_admin.id
        )
        await db_session.flush()
    ...
    @pytest.mark.asyncio
    async def test_get_chats_by_status_closed(
        self, admin_client, test_user, test_admin, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_chats_by_status_super_admin(
        self, super_admin_client, test_user, db_session
    ):
