...
- ✅ GET `/api/auth/google` - инициация OAuth2 авторизации через Google
- ✅ GET `/api/auth/google/callback` - обработка callback от Google OAuth2
...
- ✅ GET `/api/auth/telegram/init` - инициация авторизации через Telegram бот
- ✅ POST `/api/auth/telegram/authorize` - обработка авторизации пользователя через Telegram
...
- ✅ GET `/api/users/protected` - доступ к защищенному маршруту
- ✅ GET `/api/users/` - получение списка всех пользователей (только для супер-администратора)
- ✅ GET `/api/users/search` - поиск пользователей с фильтрами
- ✅ GET `/api/users/stats` - получение статистики пользователей
