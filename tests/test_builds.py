...
class TestCreateBuild:
    ...
    @pytest.mark.asyncio
    async def test_create_build_success(self, client, test_user, test_components):
...
class TestGetBuilds:
    ...
    async def test_get_builds_with_query(
        ...
        assert data["total"] >= 1
        assert any(
            "игровая" in build["title"].lower()
            or "игровая" in build["description"].lower()
...
class TestGetBuildById:
    ...
    async def test_get_build_by_id_not_found(self, client):
        ...
        assert "build not found" in response.json()["detail"].lower()
...
class TestUpdateBuild:
    ...
    async def test_update_build_success(
        ...
        update_data = {"title": "Новое название", "description": "Новое описание"}
...
class TestCreateRating:
    ...
    async def test_create_rating_success(
        ...
        response = client_user2.post(
            f"/api/builds/{created_build.id}/ratings", json=rating_data
        )
...
class TestUpdateRating:
    ...
    async def test_update_rating_success(
        ...
        create_response = client_user2.post(
            f"/api/builds/{created_build.id}/ratings", json=rating_data_create
        )
        ...
        update_response = client_user2.put(
            f"/api/builds/{created_build.id}/ratings", json=rating_data_update
        )
...
class TestDeleteRating:
    ...
    async def test_delete_rating_success(
        ...
        # Удаляем оценку
        delete_response = client_user2.delete(f"/api/builds/{created_build.id}/ratings")
...
class TestCreateComment:
    ...
    async def test_create_comment_success(
        ...
        response = client_user2.post(
            f"/api/builds/{created_build.id}/comments", json=comment_data
        )
    ...
    async def test_create_reply_comment_success(
        ...
        parent_response = client_user2.post(
            f"/api/builds/{created_build.id}/comments", json=parent_comment_data
        )
        assert parent_response.status_code == status.HTTP_201_CREATED
        parent_id = parent_response.json()["id"]

        # Создаем ответ на комментарий
        reply_data = {"content": "Ответ на комментарий", "parent_id": parent_id}

        response = client.post(
            f"/api/builds/{created_build.id}/comments", json=reply_data
        )
...
class TestUpdateComment:
    ...
    async def test_update_comment_success(
        ...
        response = client_user2.put(
            f"/api/builds/{created_build.id}/comments/{comment_id}", json=update_data
        )
