...
class TestGetPublicFeedbacks:
    ...
    @pytest.mark.asyncio
    async def test_get_public_feedbacks_with_data(
        self, unauthenticated_client, test_feedbacks
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_public_feedbacks_with_pagination(
        self, unauthenticated_client, test_feedbacks
    ):
...
class TestGetFeedbacks:
    ...
    @pytest.mark.asyncio
    async def test_get_feedbacks_with_type_filter_admin(
        self, admin_client, test_feedbacks
    ):
...
class TestGetAssignedFeedbacks:
    ...
    @pytest.mark.asyncio
    async def test_get_assigned_feedbacks_with_data(
        self, admin_client, test_feedbacks, test_admin
    ):
...
class TestGetFeedbackById:
    ...
    @pytest.mark.asyncio
    async def test_get_feedback_by_id_unauthenticated(
        self, unauthenticated_client, test_feedback
    ):
...
class TestUpdateFeedback:
    ...
    async def test_update_feedback_partial(self, client, test_feedback):
        ...
        update_data = {"title": "Частично обновленный заголовок"}
    ...
    @pytest.mark.asyncio
    async def test_update_feedback_other_user_forbidden(
        self, client_user2, test_feedback
    ):
        """Тест обновления чужого отзыва"""
        # test_feedback принадлежит test_user, пытаемся обновить его через client_user2
        update_data = {"title": "Попытка обновить чужой отзыв"}

        response = client_user2.put(
            f"/api/feedback/{test_feedback.id}", json=update_data
        )
    ...
    @pytest.mark.asyncio
    async def test_update_feedback_resolved_forbidden(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert (
            "завершенные" in response.json()["detail"].lower()
            or "отклоненные" in response.json()["detail"].lower()
        )
    ...
    @pytest.mark.asyncio
    async def test_update_feedback_unauthenticated(
        self, unauthenticated_client, test_feedback
    ):
...
class TestAdminUpdateFeedback:
    ...
    async def test_admin_update_feedback_status(self, admin_client, test_feedback):
        ...
        response = admin_client.patch(
            f"/api/feedback/{test_feedback.id}/admin", json=update_data
        )
    ...
    @pytest.mark.asyncio
    async def test_admin_update_feedback_assign(
        self, admin_client, test_feedback, test_admin
    ):
        """Тест назначения отзыва администратору"""
        update_data = {"assigned_to_id": test_admin.id}
    ...
    async def test_admin_update_feedback_response(self, admin_client, test_feedback):
        ...
        update_data = {"admin_response": "Спасибо за отзыв! Мы учтем ваши замечания."}
    ...
    @pytest.mark.asyncio
    async def test_admin_update_feedback_assign_to_non_admin(
        self, admin_client, test_feedback, test_user
    ):
        """Тест попытки назначить отзыв обычному пользователю"""
        update_data = {"assigned_to_id": test_user.id}
    ...
    @pytest.mark.asyncio
    async def test_admin_update_feedback_forbidden_for_user(
        self, client, test_feedback
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_update_feedback_unauthenticated(
        self, unauthenticated_client, test_feedback
    ):
...
class TestDeleteFeedback:
    ...
    @pytest.mark.asyncio
    async def test_delete_feedback_by_other_user_forbidden(
        self, client_user2, test_feedback
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_feedback_unauthenticated(
        self, unauthenticated_client, test_feedback
    ):
