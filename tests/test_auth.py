...
class TestGoogleAuth:
    ...
    async def test_google_callback_success(self, unauthenticated_client, db_session):
        ...
        with patch.object(
            AuthService, "_exchange_code_for_token", new_callable=AsyncMock
        ) as mock_exchange:
            with patch.object(
                AuthService, "_get_google_user_info", new_callable=AsyncMock
            ) as mock_get_user:
    ...
    async def test_google_callback_error_exchange_token(self, unauthenticated_client):
        ...
        with patch.object(
            ...
            with patch("app.config.settings") as mock_settings:
                ...
                response = unauthenticated_client.get(
                    "/api/auth/google/callback?code=test_code", follow_redirects=False
                )
...
class TestTelegramAuth:
    ...
    async def test_telegram_auth_init(self, unauthenticated_client, mock_redis_service):
        ...
        unauthenticated_client.app.dependency_overrides[get_auth_token_storage] = (
            lambda: mock_auth_token_storage
        )
        ...
        try:
            ...
            unauthenticated_client.app.dependency_overrides.pop(
                get_auth_token_storage, None
            )

    @pytest.mark.asyncio
    async def test_telegram_authorize_success(
        self, unauthenticated_client, db_session, mock_redis_service
    ):
        ...
        try:
            ...
            response = unauthenticated_client.post(
                "/api/auth/telegram/authorize", json=request_data
            )
    ...
    @pytest.mark.asyncio
    async def test_telegram_authorize_token_not_found(
        self, unauthenticated_client, mock_redis_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_telegram_authorize_token_already_used(
        self, unauthenticated_client, mock_redis_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_telegram_authorize_link_failed(
        self, unauthenticated_client, mock_redis_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_telegram_authorize_save_jwt_failed(
        self, unauthenticated_client, db_session, mock_redis_service
    ):
        ...
        mock_auth_token_storage.get_token_data = AsyncMock(return_value=token_data)
        mock_auth_token_storage.link_telegram_user = AsyncMock(return_value=True)
        mock_auth_token_storage.update_token_data = AsyncMock(
            return_value=False
        )  # Не удалось сохранить JWT
    ...
    @pytest.mark.asyncio
    async def test_telegram_auth_check_pending(
        self, unauthenticated_client, mock_redis_service
    ):
        ...
        try:
            response = unauthenticated_client.get(
                f"/api/auth/telegram/check/{auth_token}"
            )
    ...
    @pytest.mark.asyncio
    async def test_telegram_auth_check_completed(
        self, unauthenticated_client, mock_redis_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_telegram_auth_check_token_not_found(
        self, unauthenticated_client, mock_redis_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_telegram_auth_check_used_but_no_jwt(
        self, unauthenticated_client, mock_redis_service
    ):
