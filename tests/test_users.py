...
class TestGetUsers:
    ...
    async def test_get_users_with_pagination(
        self, super_admin_client, test_user, test_user2
    ):
...
class TestUpdateUserProfile:
    ...
    async def test_update_user_profile_partial(self, client, test_user):
        ...
        update_data = {"name": "New Name", "email": None, "picture": None}
...
class TestUpdateUserRole:
    ...
    async def test_update_user_role_success(self, super_admin_client, test_user):
        ...
        response = super_admin_client.put(
            f"/api/users/{test_user.id}/role", json=update_data
        )
    ...
    async def test_update_user_role_to_user(self, super_admin_client, test_admin):
        ...
        response = super_admin_client.put(
            f"/api/users/{test_admin.id}/role", json=update_data
        )
    ...
    @pytest.mark.asyncio
    async def test_update_user_role_super_admin_forbidden(
        self, super_admin_client, test_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_user_role_unauthenticated(
