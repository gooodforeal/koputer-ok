...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_builds_author_created", "builds", ["author_id", "created_at"], unique=False
    )
    ...
    op.create_index(
        "ix_build_ratings_unique", "build_ratings", ["build_id", "user_id"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["parent_id"], ["build_comments.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_build_comments_id"), "build_comments", ["id"], unique=False
    )
    op.create_index(
        "ix_build_comments_build_created",
        "build_comments",
        ["build_id", "created_at"],
        unique=False,
    )
